...
popups <- sprintf("<b>Nombre:</b> %s<br/><b>Tipo:</b> %s", proyecto_aurora$Nom_proy, proyecto_aurora$Tipo)
...
popups_pdr <- sprintf("<b>Nombre:</b> %s<br/><b>Tipo:</b> %s", proyecto_pdr$Nombre, proyecto_pdr$Tipo)
 ...
 leaflet() %>%
  ...
  addPolygons(data = proyecto_aurora, fillColor = ~paleta_aurora(etiquetas), color = "white", weight = 0.5, fillOpacity = 1, popup = popups, group = "Cartera de proyectos") %>%
  ...
  addLegend(position = "topright", pal = paleta, values = etiquetas, title = "Temática")%>%
